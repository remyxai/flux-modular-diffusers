...
| appearance_kv | default · K/V-share | ✅ recipe (spike) | non-Redux material channel — keeps the subject + applies the reference material/palette (coffee→warm, brick→grey), where Redux swamped |
...
| identity_story_material | **batch** · `inject_identity` + `share` + K/V-share | ⚠️ recipe (expressible) | 🔓 open-weight — face-lock across panels + material colour-band. DOWNGRADED: prior material reading used an unreliable colour-hist metric; batch 3-way unverified with de-risked metrics. For cross-scene cohesion use a **post-hoc colour grade** (story_capstone.ipynb), not this in-denoise op |
...
| identity_structure_material | **identity_composed** · `inject_identity` + `replace_q` + K/V colour-band | ✅ recipe (spike) | 🔓 open-weight — **three-way COMPOSES** at edge=2 (colour band) + cosine S=0.3 (n=6): identity 0.49-0.76 (bottleneck), pose d 0.83-0.96, material present. ADDITIVITY: pairwise predicts the triple. CORRECTED: the old start_frac gate made material≈0 (hollow); HF material collides with identity so keep edge low; blurred refs destabilise |

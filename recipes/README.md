...
| identity_story_material | **batch** · `inject_identity` + `share` + step-gated K/V-share | ✅ recipe (spike) | 🔓 open-weight — **art-directed story**: face-locked character + ONE shared material PALETTE across every panel. Composes identity_story with the `start_frac`-gated material; `_run_batch` carries frame-share + gated material as a composed post_rope. A100: face held 0.71–0.77 across panels + consistent palette (0.47–0.49), no dilution. See `notebooks/story_compositions.ipynb` (the 3-axis demo; garment axis = CatVTON) |
